...
// ------------------------------------------------------------------------------
//   Exactly same code as summary.ts , The only thing we change here "Prompts"
// ------------------------------------------------------------------------------
...
// - reducePrompt
// - increase tokenMax
// - shouldCollapse : Add debuuging logs
...
const llm = new ChatOpenAI({
  modelName: "llama3.1-8b",  // Fast and free
  // modelName: "llama3.1-70b", // More powerful, also free
  temperature: 0.7,
  configuration: {
...
// Increase token limit. To avoid "graph recursion limit exceed" error
...
const generateSummary = async (
  ...
  const mapPrompt = ChatPromptTemplate.fromMessages([
    ["user", 
      `
      Create a structured study notes for the following text. Includes:
      - Key concepts / definations
      - Examples of illustations
      - Important points
...
async function _reduce(input: Document[]): Promise<string> {
  const reducePrompt = ChatPromptTemplate.fromMessages([
    [
      ...
      Distill these into cohesive study guide. 
      Maintain key concepts, example and main points.
      `,
    ],
  ]);
  // Distill = शुद्ध करना
